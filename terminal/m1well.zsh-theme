...
# rendering $COLOR_WHITE background/foreground.
prompt_segment() {
  local bg fg
  [[ -n $1 ]] && bg="%K{$1}" || bg="%k"
  [[ -n $2 ]] && fg="%F{$2}" || fg="%f"
...
prompt_context() {
  if [[ "$USER" != "DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    if [ -n "${ZSH_USER_PROMPT}" ]; then
      prompt_segment $COLOR_BLACK $COLOR_WHITE "%(!.%{%F{$COLOR_YELLOW}%}.)${ZSH_USER_PROMPT}"
    else
      prompt_segment $COLOR_BLACK $COLOR_WHITE "%(!.%{%F{$COLOR_YELLOW}%}.)$USER@%m"
    fi
  fi
}
...
prompt_virtualenv() {
  ...
  if [[ -n $virtualenv_path && -n $VIRTUAL_ENV_DISABLE_PROMPT ]]; then
    prompt_segment $COLOR_BLUE $COLOR_BLACK "(`basename $virtualenv_path`)"
  fi
}
